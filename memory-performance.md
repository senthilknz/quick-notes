...
3. **The JVM heap is likely ~25% of the container limit** (JVM default `MaxRAMPercentage=25`). With a 1 Gi limit that is a ~256 MB heap — far too small for this workload.
...
### 4.4 JVM sizing vs container limit (P0)

Helm `values.yaml` (non-prod): `memory: 1Gi` request/limit. Without an explicit `-Xmx`/`-XX:MaxRAMPercentage`, JDK 17 defaults to **25% of container memory → ~256 MB heap**. Verify in a live pod:

```bash
kubectl exec <pod> -- jcmd 1 VM.flags | tr ' ' '\n' | grep -E 'MaxHeapSize|MaxRAMPercentage|G1HeapRegionSize'
```

Also confirm the prod value — the screenshot renders as `memory: "16i"`, which is presumably `1Gi` or `16Gi`; the two imply completely different risk profiles.
...
**5.3 Size the JVM explicitly** (Helm env / JAVA_TOOL_OPTIONS):
...
`ExitOnOutOfMemoryError` makes Kubernetes restart a broken pod promptly instead of letting it limp through GC thrash while failing customer requests slowly.
...
### Memory — **yes, increase (and fix the heap %)**

The bigger problem is the JVM default, not the container number: 1 Gi with a 25% heap is ~256 MB — insufficient even *after* the fixes. Recommended:
...
with `-XX:MaxRAMPercentage=75` → ~1.5 GB heap, leaving ~500 MB for metaspace, thread stacks, code cache, direct buffers, and container overhead. Budget check: 10 in-flight × ~25 MB (post-`format: byte`) + Cards guard + app baseline ≈ 700–800 MB peak — healthy headroom inside 1.5 GB. If §4.8's combined 20-permit worst case is kept, 2 Gi still holds.
